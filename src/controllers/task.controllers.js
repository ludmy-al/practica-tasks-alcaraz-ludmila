...
export const newTask = async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ ok: false, msg: "title debe de ser una cadena no vacía de máximo 100 caracteres",});
    }
    ...
    if (
      ...
      return res.status(400).json({ ok: false, msg: "description debe de ser una cadena no vacía de máximo 100 caracteres",});
    }

    if (isComplete !== undefined && typeof isComplete !== "boolean") {
      return res.status(400).json({ ok: false, msg: "isComplete debe de ser un valor booleano" });
    }
